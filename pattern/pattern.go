...
var (
	...
	// 0.01~0.99，注意0.0是不行的
	ZeroWithTwoDecimal, _ = regexp.Compile(`^(0\.[0-9]?[1-9])$`)
	...
	BetweenZeroAndThirtyWithTwoDecimal, _ = regexp.Compile(`^((0(\.[0-9]?[1-9])?)|(30)|((([1-2][0-9])|([1-9]))?(\.[0-9]?[1-9])?))$`)
	// 0.01~999.99 先搞定999.99，然后0.01
	LessThanOneThousandWithTwoDecimal, _ = regexp.Compile(`^(([1-9][0-9]{0,2}(\.[0-9]?[1-9])?)|(0\.[0-9]?[1-9]))$`)
	// 1~999的整数
	MoreThanZeroAndLessThanOneThousand, _ = regexp.Compile(`^[1-9][0-9]{0,2}$`)
	// 0~99 先搞定99.99，然后0.01
	LessThanOneHundredWithTwoDecimal, _ = regexp.Compile(`^(([1-9][0-9]?(\.[0-9]?[1-9])?)|(0\.[0-9]?[1-9]))$`)
)
